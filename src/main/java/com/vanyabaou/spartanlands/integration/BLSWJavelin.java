...
public class BLSWJavelin extends ItemJavelin implements ICorrodible, IAnimatorRepairable {
    ...
    public BLSWJavelin(String unlocName, ToolMaterialEx material) {
    ...
    public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot slot, ItemStack stack) {
        return CorrosionHelper.getAttributeModifiers(super.getAttributeModifiers(slot, stack), slot, stack,
                ItemTool.ATTACK_DAMAGE_MODIFIER, Math.max(0.5F, this.toolMaterial.getAttackDamage() * ConfigHandler.damageMultiplierJavelin + ConfigHandler.damageBaseJavelin));
    }
